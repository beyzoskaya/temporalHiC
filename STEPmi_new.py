...
class STGCN_BiLSTM_Attention(nn.Module):
    def __init__(self, stgcn, embedding_dim, hidden_dim, num_nodes, lstm_layers=1, num_heads=4, pred_len=1, dropout=0.1):
        super(STGCN_BiLSTM_Attention, self).__init__()
        self.stgcn = stgcn
    ...
    def forward(self, sequences):
        """
        sequences: list of PyG Data objects (length = seq_len)
        """
        ...
        for graph in sequences:
            ...
            out = self.stgcn(x, edge_index, edge_attr)  # shape: [num_nodes, embedding_dim]
            stgcn_outputs.append(out)
        # seq_tensor: [seq_len, num_nodes, embedding_dim]
        seq_tensor = torch.stack(stgcn_outputs, dim=0)
        # transpose to [num_nodes, seq_len, embedding_dim]
        seq_tensor = seq_tensor.transpose(0, 1)

        outputs = []
        for node_seq in seq_tensor:
            # node_seq: [seq_len, embedding_dim]
            lstm_out, _ = self.bilstm(node_seq.unsqueeze(0))  # [1, seq_len, hidden_dim*2]
            attn_out, _ = self.attention(lstm_out, lstm_out, lstm_out)
            last_out = attn_out[:, -1, :]  # [1, hidden_dim*2]
            pred = self.fc(last_out).squeeze(0)  # [pred_len]
            outputs.append(pred)
        predictions = torch.stack(outputs, dim=0)  # [num_nodes, pred_len]
        return predictions

def get_targets(label_graphs):
    # label_graphs is list of length pred_len (usually 1)
    # take first and get x: [num_nodes]
    return label_graphs[0].x
...
def evaluate_stgcn(model, data_loader, criterion, device):
    model.eval()
    total_loss = 0
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="[Eval]"):
            batch_sequences, batch_labels = batch
            batch_sequences = [[graph.to(device) for graph in seq] for seq in batch_sequences]
            batch_labels = [[graph.to(device) for graph in lbl] for lbl in batch_labels]

            batch_loss = 0
            for seq, lbl in zip(batch_sequences, batch_labels):
                preds = model(seq)
                targets = get_targets(lbl)
...
def train_stgcn(model, train_loader, test_loader, device, num_epochs=100, lr=0.001):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs} [Train]"):
            ...
            batch_loss = 0
            for seq, lbl in zip(batch_sequences, batch_labels):
                preds = model(seq)  # [num_nodes, pred_len]
                targets = get_targets(lbl)  # [num_nodes]
                # preds.squeeze(): [num_nodes]
                loss = criterion(preds.squeeze(), targets.squeeze())
                batch_loss += loss
            batch_loss = batch_loss / len(batch_sequences)
            batch_loss.backward()
        ...
        # Eval
        test_loss = evaluate_stgcn(model, test_loader, criterion, device)
