...
def train_stgcn(dataset,val_ratio=0.2):
    ...
    model = STGCNChebGraphConvProjectedGeneConnectedAttentionLSTM(args, args.blocks, args.n_vertex, gene_connections)
    model = model.float() # convert model to float otherwise I am getting type error

    optimizer = optim.Adam(model.parameters(), lr=0.0007, weight_decay=1e-5) # for mRNA data
...
if __name__ == "__main__":
    ...
    # problematic_genes = ['AMACR', 'ABCG2', 'HPGDS', 'MGAT4A', 'MMP7']
    ...
    # print('*************************************************************************************************************')
    # hic_stats, expr_stats = print_gene_analysis(dataset)
    # plot_graph_at_timepoint(dataset, -4.5)
    # plot_graph_at_timepoint(dataset, 0.5) # validation 
    # plot_graph_at_timepoint(dataset, 1.0)
    # plot_graph_at_timepoint(dataset, 1.5)
    # plot_graph_at_timepoint(dataset, 2.0)
    # plot_graph_at_timepoint(dataset, 2.5) # validation
    # plot_graph_at_timepoint(dataset, 3.0)
    # plot_graph_at_timepoint(dataset, 3.5)
    # plot_graph_at_timepoint(dataset, 4.0)
    # plot_graph_at_timepoint(dataset, 4.5)
    # plot_graph_at_timepoint(dataset, 5.0)
    # print(f"Calculating PPIs ....")
    # all_genes = list(set(dataset.df['Gene1'].unique()) | set(dataset.df['Gene2'].unique()))
    # all_ppi_data = get_mouse_ppi_data(dataset)
    # overlap_stats = compare_ppi_with_hic(dataset, all_ppi_data)
    # mgi_info = get_mgi_info(dataset)

    # os.makedirs('plottings_STGCN/ppi_analysis', exist_ok=True)

    # all_ppi_df, all_network = analyze_and_plot_ppi(dataset, 
    # save_path='plottings_STGCN/ppi_analysis/overall_ppi_network.png')

    # prob_ppi_df, prob_network = analyze_and_plot_ppi(dataset, 
    # ['AMACR', 'ABCG2', 'MMP7', 'HPGDS'],
    # save_path='plottings_STGCN/ppi_analysis/problematic_genes_ppi_network.png')

    # ppi_df, ppi_network = analyze_ppi_with_aliases(dataset, 'plottings_STGCN/ppi_analysis/alias_mapped_network.png')

    # if all_ppi_df is not None:
    #     all_ppi_df.to_csv('plottings_STGCN/ppi_analysis/overall_ppi_data.csv')
    # if prob_ppi_df is not None:
    #     prob_ppi_df.to_csv('plottings_STGCN/ppi_analysis/problematic_genes_ppi_data.csv')
