...
def map_all_chromosomes(gene_file, hic_files_dict, output_file):
    ...
    #genes_df = pd.read_csv(gene_file,sep="\t") # for mRNA files
    genes_df = pd.read_csv(gene_file)
    time_columns = validate_gene_data(genes_df)
    
    #genes_df['Chromosome'] = genes_df['Chromosome'].astype(str) for mRNA
    genes_df['Chromosome'] = genes_df['Chromosome'].astype(str).str.replace("chr", "") # for miRNA
    
    bin_size = 1000000  # 1MB bins (same for both mRNA and miRNA data)
    genes_df['Bin'] = genes_df['Start'].apply(lambda x: (x // bin_size) * bin_size)
    print("Unique Gene Bins:", genes_df['Bin'].unique()[:10])
    ...
    for chrom, hic_file in hic_files_dict.items():
        ...
        all_bins = [int(row[0]) for row in hic_array] + [int(row[1]) for row in hic_array]
        print("HiC Min Bin:", min(all_bins), "HiC Max Bin:", max(all_bins))
        print("Gene Min Bin:", genes_df['Bin'].min(), "Gene Max Bin:", genes_df['Bin'].max())


        #print(f"Head of hic_array:")
        #print(hic_array[:5])
    ...
    print("Total HiC Interactions Loaded:", len(all_hic_interactions))
    ...
    print(f"Processed chromosomes: {sorted(processed_chroms)}")
    print(f"Expected chromosomes: {sorted(hic_files_dict.keys())}")

    print("Gene Data Chromosomes:", genes_df['Chromosome'].unique())
    print("HiC Data Chromosomes:", hic_files_dict.keys())
...
if __name__ == "__main__":
    ...
    output_file = "mapped/miRNA_expression_mean/complete_genome_mapping_miRNA_expression_meaned.csv"
