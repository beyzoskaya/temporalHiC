...
from STGCN_losses import temporal_loss_for_projected_model, enhanced_temporal_loss, gene_specific_loss, miRNA_enhanced_temporal_loss
...
def train_astgcn(model, dataset, epochs=10, learning_rate=1e-3, val_ratio=0.2, device='cpu'):
    ...
    train_idx = torch.tensor([17, 100, 111, 49, 30, 124, 119, 55, 72, 142, 129, 93, 86, 103, 140, 13, 64, 108, 
                              91, 25, 135, 73, 31, 82, 48, 106, 40, 44, 130, 24, 67, 3, 102, 54, 137, 63, 71, 
                              61, 75, 59, 141, 107, 50, 112, 36, 8, 26, 11, 126, 29, 41, 74, 96, 128, 1, 33, 
                              20, 105, 52, 42, 2, 39, 109, 62, 10,  28, 87, 121, 16, 65, 122, 104, 15, 69, 147, 
                              84, 114, 58, 45, 110, 143, 144, 113, 27, 116, 6, 120, 18, 19, 32, 145, 134, 146, 
                              98, 79, 9, 132, 5, 133, 23, 47, 66, 94, 136, 22, 118, 139, 138, 70, 51, 92, 123, 
                              46, 0, 76, 14, 4, 89, 43])
    
    val_idx = torch.tensor([34, 127, 125, 21, 56, 97, 115, 85, 35, 53, 38, 57, 88, 90, 77, 60, 80, 37, 99, 101, 
                            7, 148, 83, 131, 68, 81, 12, 78, 95, 117 ])
    ...
    for epoch in range(epochs):
        ...
        for seq_graphs, label_graphs in zip(train_sequences, train_labels):
            ...
            loss = miRNA_enhanced_temporal_loss(output, y_true, x_input)
            loss.backward()
            optimizer.step()
