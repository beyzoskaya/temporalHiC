...
def train_stgcn(dataset,val_ratio=0.2):
    ...
    torch.manual_seed(42)
    model = STGCN(args, args.blocks_temporal_node2vec_with_three_st_blocks_256dim_smoother, args.n_vertex)
    model = model.float() 
...
class Args:
    def __init__(self):
        self.Kt = 10 # temporal kernel size
        self.Ks = 8 # spatial kernel size
        self.n_his = 10  # historical sequence length
        self.n_pred = 1
       
        self.blocks = [
            [256, 256, 256],    # input block
            [256, 128, 128],    # ST block
            [128, 64, 64],    # output block, hidden_dim=32
        ]
